...
class ProductsRepositoryImpl(
    ...
    override suspend fun addProductToFavorite(
       ...
       try{
           mProductDao.addProductToFavorite(product.toEntity())
       }catch (ex : Exception){
           onFailure(ex)
       }
    }
    ...
    override suspend fun removeProductFromFavorite(
        ...
        try {
            mProductDao.removeProductFromFavorite(product.toEntity())
        }catch (ex :Exception){
            onFailure(ex)
        }
    }
    ...
    override suspend fun getProducts(
        ...
        return try {
            val products = mProductDao.getProducts().map { it.toDomain() }
            onSuccess(products)
            products
        }catch (ex : Exception){
            onFailure(ex)
